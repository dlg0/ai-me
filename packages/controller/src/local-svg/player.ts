...
:root{color-scheme:dark;font-family:ui-sans-serif,system-ui,sans-serif;background:#0b1020;color:#eef2ff}*{box-sizing:border-box}body{margin:0;min-height:100vh;display:grid;place-items:center;background:radial-gradient(circle at 50% 20%,#26345d,#090d18 65%)}main{width:min(94vw,760px)}.stage{position:relative;overflow:hidden;border:1px solid #7180aa55;border-radius:24px;background:linear-gradient(150deg,#17213b,#0e1426);box-shadow:0 28px 80px #0008}.avatar{display:block;width:100%;max-height:72vh}.badge{position:absolute;top:18px;left:18px;right:18px;width:max-content;max-width:calc(100% - 36px);padding:7px 11px;border:1px solid #a9b7df55;border-radius:18px;background:#10182ddd;font-size:clamp(9px,2vw,12px);letter-spacing:.08em;text-align:center;text-transform:uppercase;overflow-wrap:anywhere}.state-label{position:absolute;z-index:1;left:50%;bottom:82px;transform:translateX(-50%);width:max-content;max-width:calc(100% - 32px);padding:7px 13px;border:2px solid #fff;border-radius:10px;background:#050814f2;color:#fff;font-size:clamp(14px,3.5vw,20px);font-weight:800;line-height:1.15;text-align:center;overflow-wrap:anywhere}.disclosure{position:absolute;left:20px;right:20px;bottom:18px;padding:11px 15px;border-radius:12px;background:#080d1be8;border-left:3px solid #7dd3fc;font-weight:650;text-align:center}.countdown{position:absolute;inset:0;display:grid;place-items:center;background:#080d1bbb;font-size:clamp(72px,18vw,150px);font-weight:800}.countdown[hidden],.panel[hidden]{display:none}.panel{display:flex;gap:10px;align-items:center;margin-top:12px;padding:12px 14px;border-radius:14px;background:#11182a}.state{flex:1}.time{font-variant-numeric:tabular-nums;color:#b8c3e2}button{border:1px solid #8392bd66;border-radius:9px;background:#24304e;color:#fff;padding:9px 15px;font-weight:650;cursor:pointer}button:hover{background:#324264}.error{color:#fda4af}@media(max-width:420px){.badge{top:12px;left:12px;right:12px;max-width:calc(100% - 24px)}.state-label{bottom:72px;max-width:calc(100% - 24px);padding:6px 10px}.disclosure{left:12px;right:12px;bottom:12px;padding:8px 10px;font-size:14px}.panel{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));padding:10px}.state{grid-column:1}.time{grid-column:2/4;text-align:right}.start{grid-column:1}.pause{grid-column:2}.restart{grid-column:3}button{padding:9px 6px}}
...
<div class="badge"></div><div class="state-label" role="status" aria-live="polite" aria-atomic="true">This is neutral.</div><div class="disclosure"></div><div class="countdown" hidden>3</div></section><section class="panel"><div class="state"><strong class="status">Ready</strong><div class="current">neutral</div></div><span class="time">00:00.0 / 00:00.0</span><button class="start">Start</button><button class="pause" disabled>Pause</button><button class="restart">Restart</button></section></main>
<script id="playback-data" type="application/json">${json}</script><script>
'use strict';const data=JSON.parse(document.getElementById('playback-data').textContent),visual=data.visualMapping;const $=s=>document.querySelector(s),head=$('#head'),eyes=$('#eyes'),pupils=$('#pupils'),brows=$('#brows'),mouth=$('#mouth'),mouthOpen=$('#mouth-open'),caveat=$('#caveat'),boundary=$('#boundary'),status=$('.status'),current=$('.current'),stateLabel=$('.state-label'),time=$('.time'),countdown=$('.countdown'),start=$('.start'),pause=$('.pause');$('.badge').textContent=data.metadata.rigId+' · offline prototype';$('.disclosure').textContent=data.metadata.disclosure;
let mode='ready',started=0,pausedAt=0,pauseTotal=0,last=-1,raf=0,nextStartAt=null;const f=n=>{const s=((n%60000)/1000).toFixed(1).padStart(4,'0');return String(Math.floor(n/60000)).padStart(2,'0')+':'+s};const setNeutral=()=>applyControls(data.neutralControls);
function displayState(value,isError=false){const raw=value||'neutral';current.textContent=isError?'error':raw;stateLabel.textContent=isError?'Playback error.':'This is '+String(raw).replace(/_/g,' ')+'.'}
function applyControls(c){const v=id=>c[id],open=v('avatar-mouth');head.setAttribute('transform','translate('+(v('avatar-head-x')*visual.headTranslateX)+' '+(v('avatar-head-y')*visual.headTranslateY)+') rotate('+v('avatar-head-z')+' 320 300)');pupils.setAttribute('transform','translate('+(v('avatar-gaze-x')*visual.gazeTranslateX)+' '+(v('avatar-gaze-y')*visual.gazeTranslateY)+')');eyes.setAttribute('transform','translate(0 '+(294*(1-v('avatar-eyes')))+') scale(1 '+v('avatar-eyes')+')');brows.setAttribute('transform','translate(0 '+(-v('avatar-brows')*visual.browTranslateY)+')');mouth.setAttribute('d','M282 362 Q320 '+(378+v('avatar-smile')*visual.smileCurve-open*visual.mouthCurve)+' 358 362');mouth.setAttribute('stroke-width',String(visual.mouthStroke+open*visual.mouthStroke));mouthOpen.setAttribute('ry',String(open*visual.mouthOpenRadius));mouthOpen.setAttribute('opacity',String(Math.min(1,open*visual.mouthOpenOpacity)));caveat.setAttribute('opacity',String(Math.min(1,v('avatar-caveat')*visual.caveatOpacity)));boundary.setAttribute('opacity',String(v('avatar-boundary')))}
function begin(){if(mode==='countdown'||mode==='playing'||mode==='paused'||mode==='paused-countdown')return;restart()}
function restart(){cancelAnimationFrame(raf);setNeutral();displayState(null);status.classList.remove('error');mode='countdown';const delay=typeof nextStartAt==='number'?Math.max(0,nextStartAt-Date.now()):3000;nextStartAt=null;started=performance.now()-(3000-delay);pauseTotal=0;last=-1;countdown.hidden=false;countdown.textContent=String(Math.max(1,Math.ceil(delay/1000)));status.textContent='Countdown';start.disabled=true;pause.disabled=false;pause.textContent='Pause';time.textContent=f(0)+' / '+f(data.metadata.durationMs);raf=requestAnimationFrame(tick)}
function tick(now){try{if(mode==='paused'||mode==='paused-countdown'){raf=requestAnimationFrame(tick);return}const elapsed=now-started-pauseTotal;if(mode==='countdown'){const left=3000-elapsed;if(left>0){countdown.textContent=String(Math.ceil(left/1000));raf=requestAnimationFrame(tick);return}mode='playing';started=now;pauseTotal=0;countdown.hidden=true;status.textContent='Playing'}const at=now-started-pauseTotal;if(at>=data.metadata.durationMs){setNeutral();displayState(null);mode=data.metadata.outcome;status.textContent=data.metadata.outcome==='completed'?'Completed':data.metadata.outcome==='cancelled'?'Cancelled':'Error';status.classList.toggle('error',data.metadata.outcome==='error');start.disabled=false;pause.disabled=true;time.textContent=f(data.metadata.durationMs)+' / '+f(data.metadata.durationMs);return}let i=Math.min(data.frames.length-1,Math.floor(at/(data.renderScript[0].tickMs)));if(i!==last){last=i;const frame=data.frames[i];applyControls(frame.controls);displayState(frame.currentState)}time.textContent=f(at)+' / '+f(data.metadata.durationMs);raf=requestAnimationFrame(tick)}catch(e){setNeutral();displayState(null,true);mode='error';status.textContent='Error';status.classList.add('error');start.disabled=false;pause.disabled=true}}
function togglePause(wantPaused){const isPaused=mode==='paused'||mode==='paused-countdown';if(wantPaused===isPaused)return;if(mode==='playing'||mode==='countdown'){pausedAt=performance.now();mode=mode==='countdown'?'paused-countdown':'paused';status.textContent='Paused';pause.textContent='Resume'}else if(isPaused){const wasCountdown=mode==='paused-countdown';pauseTotal+=performance.now()-pausedAt;mode=wasCountdown?'countdown':'playing';status.textContent=wasCountdown?'Countdown':'Playing';pause.textContent='Pause'}}
pause.addEventListener('click',()=>togglePause(!(mode==='paused'||mode==='paused-countdown')));start.addEventListener('click',begin);$('.restart').addEventListener('click',restart);window.addEventListener('message',event=>{const m=event.data;if(event.source!==parent||!m||typeof m!=='object'||Array.isArray(m)||Object.keys(m).some(k=>!['protocol','command','startAtEpochMs'].includes(k))||m.protocol!=='local-svg-player-control.v1'||!['start','pause','resume','restart'].includes(m.command))return;const timed=m.command==='start'||m.command==='restart';if(timed&&(!Number.isSafeInteger(m.startAtEpochMs)||m.startAtEpochMs<Date.now()))return;if(!timed&&'startAtEpochMs'in m)return;if(timed){nextStartAt=m.startAtEpochMs;restart()}else togglePause(m.command==='pause')});if(window.parent!==window){document.querySelector('.panel').hidden=true}document.addEventListener('visibilitychange',()=>{if(document.hidden&&(mode==='playing'||mode==='countdown'))togglePause(true)});setNeutral();time.textContent=f(0)+' / '+f(data.metadata.durationMs);
</script></body></html>\n`;
}
