...
export function generateLocalSvgPlayer(planInput: unknown, profileInput: unknown): string {
  ...
  const payload = { metadata: { title: plan.title, durationMs: plan.durationMs, rigId: profile.rigId, assetId: profile.avatar.assetId, disclosure: disclosure.text }, renderScript: records, frames, neutralControls: localSvgNeutralControls(profile) };
...
'use strict';const data=JSON.parse(document.getElementById('playback-data').textContent);const $=s=>document.querySelector(s),head=$('#head'),eyes=$('#eyes'),pupils=$('#pupils'),brows=$('#brows'),mouth=$('#mouth'),caveat=$('#caveat'),boundary=$('#boundary'),status=$('.status'),current=$('.current'),time=$('.time'),countdown=$('.countdown'),start=$('.start'),pause=$('.pause');$('.badge').textContent=data.metadata.rigId+' · offline prototype';$('.disclosure').textContent=data.metadata.disclosure;
...
function applyControls(c){const v=id=>c[id];head.setAttribute('transform','translate('+v('avatar-head-x')+' '+v('avatar-head-y')+') rotate('+v('avatar-head-z')+' 320 300)');pupils.setAttribute('transform','translate('+(v('avatar-gaze-x')*14)+' '+(v('avatar-gaze-y')*12)+')');eyes.setAttribute('transform','translate(0 '+(294*(1-v('avatar-eyes')))+') scale(1 '+v('avatar-eyes')+')');brows.setAttribute('transform','translate(0 '+(-v('avatar-brows')*12)+')');mouth.setAttribute('d','M282 362 Q320 '+(378+v('avatar-smile')*28-v('avatar-mouth')*18)+' 358 362');mouth.setAttribute('stroke-width',String(12+v('avatar-mouth')*12));caveat.setAttribute('opacity',String(v('avatar-caveat')));boundary.setAttribute('opacity',String(v('avatar-boundary')))}
